...
% contained main.m requests an SNR vaule by the user, loads microphone
% signals, estimates early PSDs and updates RETFs using [2] (see also
% https://github.com/tdietzen/SQRT-PSD-RETF), runs the ISCLP Kalman filter,
% and plays back the enhanced signal.
...
%%% PSD estimation and RETF update [2]
...
%% EARLY PSD ESTIMATION, RETF UPDATE [2]

fprintf(' * estimate early PSDs, update RETFs [2]...\n');
...
fprintf(' * run ISCLP Kalman filer [1]...\n');

% number of frames
numFrames = size(y_STFT,2);

% init outputs
q_STFT              = zeros(N_STFT_half, numFrames);
e_prio_STFT         = zeros(N_STFT_half, numFrames);
e_post_STFT         = zeros(N_STFT_half, numFrames);
e_post_smooth_STFT  = zeros(N_STFT_half, numFrames);
...
% plot
figure('Name',['enhanced signal, beta = 0, SNR = ' num2str(SNR) 'dB']);
plotSpec(e_post_STFT(:,:,1),        'mag', xTickProp, yTickProp, cRange, 0); title(['e post, beta = 0, SNR = ' num2str(SNR) 'dB']); ylabel('f/kHz');
figure('Name',['enhanced signal, beta = ' num2str(round(beta_SCLP_KF,2)) ', SNR = ' num2str(SNR) 'dB']);
plotSpec(e_post_smooth_STFT(:,:,1), 'mag', xTickProp, yTickProp, cRange, 0); title(['e post, beta = ' num2str(round(beta_SCLP_KF,2)) ', SNR = ' num2str(SNR) 'dB']); ylabel('f/kHz');
drawnow;


%% ISTFT PROCESSING

e_post_TD        = calc_ISTFT(e_post_STFT, win, N_STFT, R_STFT, 'onesided');
e_post_smooth_TD = calc_ISTFT(e_post_smooth_STFT, win, N_STFT, R_STFT, 'onesided');
...
audiowrite(['.' filesep 'audio' filesep 'e_post_SNR_' num2str(SNR) 'dB.wav'],e_post_TD,fs);
audiowrite(['.' filesep 'audio' filesep 'e_post_smooth_SNR_' num2str(SNR) 'dB.wav'],e_post_smooth_TD,fs);

fprintf(' * play microphone signal...\n'); 
sound(y_TD(:,1), fs); pause(length(y_TD(:,1))/fs);
fprintf(' * play enhanced signal...\n'); 
sound(e_post_smooth_TD(:,1), fs);
